...
// Custom error for missing preview/copy
export class PreviewNotGeneratedError extends Error {
  constructor(businessName?: string) {
    super(businessName ? `Preview not generated for "${businessName}"` : 'Preview not generated');
    this.name = 'PreviewNotGeneratedError';
  }
}
...
export async function fetchPreviewData(idOrSlug: string): Promise<PreviewData> {
  ...
  // Check if copy has been generated by checking for hero headline
  if (!business.copy_hero_headline) {
    throw new PreviewNotGeneratedError(business.business_name);
  }
