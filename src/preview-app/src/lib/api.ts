 ...
 * Uses the on-demand preview endpoint that automatically generates missing steps
 * This may take 10-15 seconds for uncached previews (LLM generation)
 */
export async function fetchPreviewData(idOrSlug: string): Promise<PreviewData> {
  // Use the new on-demand preview endpoint
  // This endpoint handles generation automatically if needed
  const response = await fetch(`${API_BASE_URL}/preview/${encodeURIComponent(idOrSlug)}`);
